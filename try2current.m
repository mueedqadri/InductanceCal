...
for i = 1: size_t
    if i > 1
        ...
        Va(i) = 220*sqrt(2)*sin(2*pi*f*t(i)); % replace with the actual function
        Vb(i) = 220*sqrt(2)*sin(2*pi*f*t(i)-(120*pi/180)); % replace with the actual function
        Vc(i) = 220*sqrt(2)*sin(2*pi*f*t(i)+(120*pi/180));
        Vs(:, :, i) = [Va(i);Vb(i);Vc(i)];
